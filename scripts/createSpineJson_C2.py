...
def getAnimationList(slotList,boneList,fps,start,end,offsetRange):
    ...
    for slot in slotList:
       ...
       # print slot, slotFileDict[slot]
        alphaGainkeyFrameList = cmds.keyframe(fileNode, query=True)
...
def runB():
    ...
    rootName = "bigShineGrp"
    fileName = "C:/alphaOnly/github/JS_learning/spineTest/sunset_king/bigWin_effectA_03.json"
    boneList = getAllbones(rootName)
    slotList = getAllSlots(boneList)
    skinList = getSkinsList(slotList)
    animationList = getAnimationList(slotList,boneList,30,0,200,0)
