        ...
        event.adjustAnimTransform = function (effectList, effectName, effectContainer, eventTick, adjustList) {
            ...
            for (i in effectContainer) {
              ...
              //  var adjustBlur = PIXI_globalData[effectName].adjust.spritesBlur;
                ...
                var offsetEventTick =  (offset + eventTick)%timeLength+1;
                /*
                if ((offset - eventTick) > 0) {
                    var offsetEventTick = offset + eventTick;
                    //var trans = 0;
                   // console.log("offsetEventTick",instancerKey,offsetEventTick)
                }else if((offset - eventTick) == 0){
                    var offsetEventTick = offset+eventTick+1;
                    //var trans = 0;
                   // console.log("offsetEventTick",instancerKey,offsetEventTick)
                }else {
                    var offsetEventTick =  (offset + eventTick)%timeLength+1;
                ...
                */
                ...
                for (j in effectContainer[i].children) {
                    ...
                    if (imageIndex == null) {
                        ...
                        pp.gotoAndPlay(imageIndex);
                        //console.log("frameIndex index",imageIndex)
                    };
                    if (motionData.metaData.sequence == false) {
                        pp.animationSpeed = 0
                    } else {
...
function addEffectFiles() { //loadFile 由PIXI_fileInput_module_min
    var effectCount = Object.keys(loadFiles).length
    var runLoadEffect = loadEffect.createNew();

    for (i in Object.keys(loadFiles)) {
        let effectName = Object.keys(loadFiles)[i];
        //loadFiles[effectName]
        let motionID = loadFiles[effectName].motionID;
        let motionURL = loadFiles[effectName].motionURL;
        let spritesID = loadFiles[effectName].spritesID;
        let spritesURL = loadFiles[effectName].spritesURL;
        runLoadEffect.loadData(motionID, motionURL, spritesID, spritesURL, effectName);
...
function loadEffectFiles() {
    ...
    for (i in Object.keys(loadFiles)) {
        ...
        let spritesID = loadFiles[effectName].spritesID;
        let spritesURL = loadFiles[effectName].spritesURL;
        runLoadEffect.readData(motionID, motionURL, spritesID, spritesURL, effectName);
    };
...
function runEffect() {
    ...
    var effectCount = Object.keys(loadFiles).length
    var runLoadEffect = loadEffect.createNew();
    var defineEffect = defineSpriteEvent.createNew();
    for (i in Object.keys(loadFiles)) {
        let effectName = Object.keys(loadFiles)[i];
        let adjustList = loadFiles[effectName];
        let containerName = "container_" + effectName;
        console.log("effectList",effectList,effectName,containerName,adjustList)

        defineEffect.define(effectList, effectName);
        let effectContainer = appStageContainers.effects[containerName];
        console.log("effectContainer",effectContainer)
        defineEffect.runApp(effectList, effectName, effectContainer, adjustList);
    };
};
